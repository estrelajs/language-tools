...
export class TsPlugin {
  ...
  static isEnabled(): boolean {
    return (
      // workspace.getConfiguration('estrela').get<boolean>('enable-ts-plugin') ??
      true
    );
  }
  ...
  async askToEnable() {
    ...
    const response = await window.showInformationMessage(
      'The Estrela for VSCode extension now contains a TypeScript plugin. ' +
        'Enabling it will provide intellisense for Estrela files from TS/JS files. ' +
